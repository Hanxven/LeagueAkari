...
- 自动开启匹配功能将可以自行中断，根据设置策略。

## 调整

- 平衡性 BUFF 将提供颜色边框。

- 现在 LCU 连接行为将会在主进程进行，而不是依赖于渲染进程，这样可以提高相应速度。

- 现在将不依赖管理员权限启动。以管理员身份启动将实现更多功能。

## 修复

- [UNFINISHED / UNPUBLISHED] 在低优先级队列时，自动取消重复执行的问题。

- [UNFINISHED / UNPUBLISHED] 在秒退惩罚时，自动匹配的问题。

- [UNFINISHED] 自动点赞相关功能问题。
