...
- 提供了一个基于键盘输入模拟的字符串发送机制，现在可以在游戏中发送自定义的文本，使用 `Delete` 快捷键。

- 提供了一个功能，可以在游戏结束时重新回到房间。位于 `自动化` -> `流程` -> `游戏结算时` -> `自动回到房间开启`。

- [UNTESTED] 现在自动点赞并入到 `游戏结算时` 模块中。
