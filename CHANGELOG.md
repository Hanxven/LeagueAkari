...
- [UNFINISHED] 重构了项目结构。现在一些核心的状态更新移动至主进程。得益于此，更多的功能将能被实现。

## 新增

- [UNFINISHED] 在英雄选择过程中，提供一个额外小窗口，用于指示即将进行的自动化操作。

- [UNFINISHED] 现在可以标记玩家，并在下次遇到时加以提醒。

- [UNFINISHED] 提供了一个标题栏便捷选项，当多客户端存在时，可快捷更换连接的客户端。

- [UNFINISHED] 提供了一个基于键盘输入模拟的自定义发送序列。

- [UNFINISHED] 现在 League Akari 会记录日志，日志将写入到程序目录下的 `logs/LeagueAkari_{datetime}.log`。可以通过 `设置` -> `调试` -> `日志` -> `日志目录` 来查看日志文件。

- [NEXT UNFINISHED] 提供自动符文相关功能。

## 调整

- [UNFINISHED] 现在大部分自动化功能将列入额外的面板中。

- 由于一些依赖问题，使用 yarn 作为包管理工具。

- [UNFINISHED] 现在使用 SQLite3 数据库来记录大部分数据（而不是基于原本的渲染进程的 Local Storage）。由于之前记录数据不兼容，会丢失一些原本的数据。但设置项会被正确地迁移。
