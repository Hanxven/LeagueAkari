...
export class WindowManagerMain implements IAkariShardInitDispose {
  ...
  static MAIN_WINDOW_DEFAULT_SIZE = [1256, 780] as [number, number]
  ...
  private _createAuxWindow() {
    ...
    if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
      this._aw.loadURL(`${process.env['ELECTRON_RENDERER_URL']}/auxiliary-window.html`)
    } else {
      this._aw.loadFile(join(__dirname, '../renderer/auxiliary-window.html'))
    }
  ...
  private _handleAuxWindowObservations() {
    ...
    this._mobx.reaction(
      ...
      (f) => {
        this._setting._saveToStorage('functionality', f)
      }
    )
